...
def create_direct_challenge(request, team_id):
            ...
            if not team.eligible:
                ...
                if time_difference < 1200:  # 1200 seconds = 20 minutes
                    ...
                    if has_disputes:
                        ...
                        challenge = form.save(commit=False)
                        challenge.challenging_team = request.user.current_team  # Assuming field name as 'challenging_team'
    ...
    return render(request, 'create_direct_challenge.html', context)
...
def accept_challenge(request, challenge_id):
    ...
    if not team.eligible:
        ...
        if request.user.current_team.full_team and request.user == request.user.current_team.owner and not challenge.accepted:
            team2 = request.user.current_team
...
def accept_direct_challenge(request, direct_challenge_id):
    ...
    if not team.eligible:
        ...
        if request.user.current_team.full_team and request.user == request.user.current_team.owner and not direct_challenge.accepted:
...
def challenges(request):
    ...
    if request.user.is_authenticated:
        ...
        challenges = Challenge.objects.filter(accepted=False)
...
def match_details(request, match_id):
    ...
    if isinstance(check_result, HttpResponseRedirect):
        ...
        return render(request, 'match_details.html', {'match': match, 'is_match_over': is_match_over})

def update_wins_and_losses (request, match_id):
    match = get_object_or_404(Match, pk=match_id)
...
def submit_results(request, match_id):
    match = get_object_or_404(Match, pk=match_id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if match_results_count == 2:
                ...
                team2_result = MatchResult.objects.get(match=match, team_owner=match.team2.owner).team_result
                ...
                # Update the Match model with the final results
                    ...
                    update_wins_and_losses(request, match_id)
                    ...
                    return redirect('dispute_proofs_list')
    ...
    return render(request, 'submit_results.html', {'form': form})

def update_dispute_proof(request, proof_id):
    dispute_proof = get_object_or_404(DisputeProof, pk=proof_id)
    match = dispute_proof.match
    dispute_proof1 = DisputeProof.objects.get(owner=match.team1.owner, match=match)
    dispute_proof2 = DisputeProof.objects.get(owner=match.team2.owner, match=match)
    ...
    if time_since_dispute < timezone.timedelta(hours=1):
        if request.method == 'POST':
            form = DisputeProofForm(request.POST, request.FILES, instance=dispute_proof)
            if form.is_valid():
                form.save()
                ...
                other_proof = DisputeProof.objects.filter(match=dispute_proof.match).exclude(owner=dispute_proof.owner).first()

                if other_proof:
                    # Create an instance of the Dispute model only if it doesn't exist
                    dispute, created = Dispute.objects.get_or_create(match=dispute_proof.match)
                    ...
                    return redirect('update_dispute_proof', proof_id=proof_id)

        else:
            form = DisputeProofForm(instance=dispute_proof)

        return render(request, 'update_dispute_proof.html', {'form': form, 'match': dispute_proof.match})
    else:
...
def dispute_proofs_list(request):
    # Get all DisputeProof instances associated with the current user
    dispute_proofs = DisputeProof.objects.filter(owner=request.user)

    return render(request, 'dispute_proofs_list.html', {'dispute_proofs': dispute_proofs})

@user_passes_test(is_admin)
def dispute_proof_details(request, proof_id):
    dispute_proof = get_object_or_404(DisputeProof, pk=proof_id)
    return render(request, 'dispute_proof_details.html', {'dispute_proof': dispute_proof})
...
def dispute_details(request, dispute_id):
    ...
    if request.method == 'POST':
        ...
        #update wins and losses
        update_wins_and_losses(request, match.id)
        
        # Get the MatchResult instances for the match
        team1_match_result = MatchResult.objects.get(match=match, team_owner=match.team1.owner)
        team2_match_result = MatchResult.objects.get(match=match, team_owner=match.team2.owner)
        ...
        return redirect('dispute_details', dispute_id=dispute_id)

    return render(request, 'dispute_details.html', {'dispute': dispute, 'match': match})
...
def my_challenges_view(request):
    ...
    if request.user.is_authenticated and hasattr(request.user, 'current_team'):
        team = request.user.current_team
        ...
        my_challenged_direct_challenges = DirectChallenge.objects.filter(challenged_team=team)
...
def schedule_conflict(request):
    return render(request, 'schedule_conflict.html')

def match_farming(request):
    return render(request, 'match_farming.html')

def decline_direct_challenge(request, challenge_id):
    # Retrieve the direct challenge instance
    direct_challenge = get_object_or_404(DirectChallenge, pk=challenge_id)
    ...
    return redirect('my_challenges')  # Adjust 'my_challenges' to your desired URL name

def submit_results_success(request):
    return render(request, 'submit_results_success.html')

def support_request_success(request):
...
def team_not_eligible(request):
    return render(request, 'team_not_eligible.html')
        ...
        if match_results.exists():
            ...
            if not match.match_completed:
                if team1_result and not team2_result:
                    ...
                    match.match_completed = True
                    match.save()
                    return HttpResponseRedirect(reverse('match_details', kwargs={'match_id': match_id}))
...
def request_match_support(request, match_id):
    match = Match.objects.get(id=match_id)

    if request.user == match.team1.owner or request.user == match.team2.owner:
        if request.method == 'POST':
            form = MatchSupportForm(request.POST)
            if form.is_valid():
        ...
        return render(request, 'support_request_form.html', context)
    else: 
